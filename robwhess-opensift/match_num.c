...
void* process_image(void* arg) {
  ...
  if (!img);
	  //fatal_error("Unable to load image from %s", ctx->filename);
...
int main( int argc, char** argv ) {
  ...
  for (i=0; i < 2; ++i) {
	...
	if (rc);
		//fatal_error("Unable to create processing thread (err %d).",rc);
  }
  ...
  for (i=0; i < 2; ++i) {
	...
	if (rc);
		//fatal_error("Return code from thread pthread_join is %d",rc);
  }
